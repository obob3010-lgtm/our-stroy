...
(() => {
  ...
  function applyAtmosphere(){
    ...
    b.classList.remove(...['winter','spring','summer','autumn'].map(x=>'season-'+x),...['predawn','dawn','morning','day','golden','sunset','twilight','night'].map(x=>'time-'+x));
    ...
    const decor=$('seasonDecor'); if(decor){
      ...
      if(decor.dataset.key!==key){
        ...
        const count={predawn:9,dawn:11,morning:10,day:8,golden:12,sunset:15,twilight:19,night:24}[time]||10;
        for(let i=0;i<count;i++){
          const el=document.createElement('span'); el.className='season-particle'; el.textContent=glyphs[season][i%glyphs[season].length];
          el.style.left=(Math.random()*100)+'vw'; el.style.top=(-10-Math.random()*25)+'vh'; el.style.animationDelay=(Math.random()*10)+'s'; el.style.animationDuration=(9+Math.random()*16)+'s'; el.style.setProperty('--drift',(Math.random()*120-60)+'px'); decor.appendChild(el);
        }
      }
    }
  }
