...
(() => {
  ...
  // ---------- Cinematic atmosphere: Tashkent UTC+5 ----------
  ...
  function leaf(el,i){
    el.className='falling-leaf leaf-'+((i%5)+1); el.innerHTML='<span class="leaf-vein"></span>';
    el.style.left=(Math.random()*112-6)+'vw'; el.style.top=(-18-Math.random()*35)+'vh';
    el.style.setProperty('--size',(44+Math.random()*68)+'px'); el.style.setProperty('--drift',(Math.random()*44-22)+'vw');
    el.style.setProperty('--spin',(Math.random()>0.5?'':'-')+(220+Math.random()*420)+'deg');
    el.style.animationDelay=(-Math.random()*18)+'s'; el.style.animationDuration=(11+Math.random()*13)+'s';
  }
  function makeAtmoDecor(season,time){
    const decor=$('seasonDecor'); if(!decor)return; decor.replaceChildren();
    if(season==='autumn'){
      const count=time==='night'||time==='twilight'?34:46;
      for(let i=0;i<count;i++){const el=document.createElement('span');leaf(el,i);decor.appendChild(el);}
      for(let i=0;i<9;i++){const el=document.createElement('span');el.className='autumn-depth-leaf';el.style.left=(Math.random()*100)+'vw';el.style.top=(Math.random()*100)+'vh';el.style.setProperty('--s',(1.2+Math.random()*2.2));el.style.setProperty('--rot',(Math.random()*180-90)+'deg');decor.appendChild(el);}
    }else if(season==='winter'){
      for(let i=0;i<30;i++){const el=document.createElement('span');el.className='snowflake';el.textContent=['·','❄','✦'][i%3];el.style.left=Math.random()*100+'vw';el.style.animationDelay=(-Math.random()*14)+'s';el.style.animationDuration=(10+Math.random()*12)+'s';el.style.fontSize=(8+Math.random()*14)+'px';decor.appendChild(el);}
    }else if(season==='spring'){
      for(let i=0;i<22;i++){const el=document.createElement('span');el.className='spring-petal';el.textContent='✦';el.style.left=Math.random()*100+'vw';el.style.animationDelay=(-Math.random()*15)+'s';el.style.animationDuration=(12+Math.random()*10)+'s';el.style.fontSize=(8+Math.random()*12)+'px';decor.appendChild(el);}
    }else{
      for(let i=0;i<18;i++){const el=document.createElement('span');el.className='summer-firefly';el.style.left=Math.random()*100+'vw';el.style.top=(15+Math.random()*80)+'vh';el.style.animationDelay=(-Math.random()*5)+'s';decor.appendChild(el);}
    }
  }
  function applyAtmosphere(){
    const n=tashkentNow(),sm=String(CFG.seasonMode||'auto').toLowerCase(),tm=String(CFG.timeMode||'auto').toLowerCase();
    const aliases={dusk:'sunset','pre-dawn':'predawn'};
    ...
    const b=document.body;b.classList.remove(...['winter','spring','summer','autumn'].map(x=>'season-'+x),...['predawn','dawn','morning','day','golden','sunset','twilight','night'].map(x=>'time-'+x));b.classList.add('season-'+season,'time-'+time);
    const badge=$('seasonBadge');if(badge)badge.textContent=`${seasonNames[season]} · ${timeNames[time]} · ${String(n.hour).padStart(2,'0')}:${String(n.minute).padStart(2,'0')}`;
    const key=season+'-'+time;if($('seasonDecor')?.dataset.key!==key){$('seasonDecor').dataset.key=key;makeAtmoDecor(season,time);}
  }
  ...
  function normalizeMemory(row){return{ id:row.id,roomId:row.room_id,date:row.date_label||'',title:row.title||'',text:row.body||'',photos:Array.isArray(row.photo_paths)?row.photo_paths:[],videos:Array.isArray(row.video_paths)?row.video_paths:[],youtubeVideos:Array.isArray(row.youtube_videos)?row.youtube_videos:[],audioPath:row.audio_path||'',createdAt:new Date(row.created_at||Date.now()).getTime(),createdBy:row.created_by||null,cloud:true };
  }
  ...
  async function loadMemories(){
    ...
    const {data,error}=await supabaseClient.from(TABLE).select('id,room_id,date_label,title,body,photo_paths,video_paths,youtube_videos,audio_path,created_at,created_by').eq('room_id',ROOM).order('created_at',{ascending:false});
  ...
  function parseYouTube(url){
    ...
    try{
      ...
      return {video_id:id,url:raw,title:'',description:'',thumbnail:`https://i.ytimg.com/vi/${id}/hqdefault.jpg`};
    }catch{return null;}
  }
  async function enrichYouTube(item){
    try{
      const r=await fetch(`https://www.youtube.com/oembed?url=${encodeURIComponent('https://www.youtube.com/watch?v='+item.video_id)}&format=json`,{headers:{'Accept':'application/json'}});
      if(r.ok){const d=await r.json();item.title=d.title||item.title;item.author_name=d.author_name||'';item.thumbnail=d.thumbnail_url||item.thumbnail;}
    }catch{}
    return item;
  }
  function youtubeCard(v,compact=false){
    const title=esc(v.title||'YouTube-видео'), thumb=v.thumbnail||`https://i.ytimg.com/vi/${v.video_id}/hqdefault.jpg`;
    return `<article class="yt-card ${compact?'compact':''}"><button class="yt-poster" type="button" data-youtube="${esc(v.video_id)}"><img src="${esc(thumb)}" alt=""><span class="yt-play">▶</span></button><div class="yt-card-meta"><span class="eyebrow">YouTube</span><h4>${title}</h4>${v.author_name?`<span>${esc(v.author_name)}</span>`:''}</div></article>`;
  }
  function youtubeEmbed(id){return `<div class="youtube-player"><iframe src="https://www.youtube.com/embed/${encodeURIComponent(id)}?rel=0&modestbranding=1&playsinline=1" title="YouTube video" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`;}
  function bindYouTubePosters(root=document){root.querySelectorAll('[data-youtube]').forEach(b=>b.addEventListener('click',()=>{const id=b.dataset.youtube;const card=b.closest('.yt-card');if(card)card.outerHTML=youtubeEmbed(id);}));}
  ...
  async function initMuseum(){
    ...
    const photosInput=$('entryPhotos'),youtubeInput=$('entryYoutube'),audioInput=$('entryAudio'),preview=$('entryPreview'),ytPreview=$('youtubePreview'),progress=$('entryProgress');
    function status(kind,text){const x=$('museumStatus');x.className='museum-status '+kind;$('museumStatusText').textContent=text;}
    function updateStats(){const stats=$('museumStats');const monthStart=new Date();monthStart.setDate(1);monthStart.setHours(0,0,0,0);const thisMonth=memories.filter(e=>e.createdAt>=monthStart.getTime()).length;const latest=memories[0]?.createdAt;const latestText=latest?new Date(latest).toLocaleDateString('ru-RU',{day:'numeric',month:'long'}):'пока нет';const fresh=lastVisit?memories.filter(e=>e.createdAt>lastVisit).length:0;const videoCount=memories.reduce((n,e)=>n+(e.youtubeVideos||[]).length+(e.videos||[]).length,0);stats.innerHTML=`<span>${memories.length} ${memories.length===1?'глава':'глав'}</span><span>${videoCount} видео</span><span>${thisMonth} в этом месяце</span><span>последняя · ${esc(latestText)}</span>${fresh?`<span>+${fresh} новых</span>`:''}`;const f=memories[0];$('museumFeatureTitle').textContent=f?.title||'здесь пока тихо';$('museumFeatureText').textContent=f?.text||'Добавьте первое воспоминание — и музей начнёт жить.';}
    function render(){grid.innerHTML='';$('museumEmpty').style.display=memories.length?'none':'block';for(const e of memories){const isNew=lastVisit&&e.createdAt>lastVisit;const c=document.createElement('article');c.className='museum-card'+(isNew?' is-new':'');const cover=e.media?.photos?.[0]||'';const yt=(e.youtubeVideos||[]).length;const badges=[e.photos.length?`фото ${e.photos.length}`:'',e.videos.length?`старое видео ${e.videos.length}`:'',yt?`YouTube ${yt}`:'',e.audioPath?'♫ песня':''].filter(Boolean).join(' · ');c.innerHTML=`<div class="museum-card-photo">${cover?`<img src="${cover}" alt="" loading="lazy">`:'<span class="museum-no-photo">✦</span>'}</div><span class="museum-card-sync">${isNew?'новое · ':''}${badges||'глава'}</span><div class="museum-card-body"><div class="museum-card-date">${esc(e.date||new Date(e.createdAt).toLocaleDateString('ru-RU'))}</div><div class="museum-card-title">${esc(e.title||'без названия')}</div><div class="museum-card-text">${esc(e.text||'')}</div></div>`;c.addEventListener('click',()=>openView(e));grid.appendChild(c)}updateStats();if(memories.length)safeSet(LAST_VISIT_KEY,String(Date.now()));renderCinema();}
    async function refresh(){status('','синхронизирую…');try{memories=await loadMemories();render();status('online','облако синхронизировано')}catch(e){console.error(e);memories=backupRead().map(x=>({...x,cloud:false,youtubeVideos:x.youtubeVideos||[]}));render();status('error','облако недоступно · резервная копия')}}
    function openAdd(){['entryDate','entryTitle','entryText','entryYoutube'].forEach(id=>$(id).value='');photosInput.value='';audioInput.value='';preview.innerHTML='';ytPreview.innerHTML='';progress.textContent='';modal.classList.add('show');}
    async function previewUploads(){preview.innerHTML='';const photos=[...photosInput.files].slice(0,MAX_IMAGES),audio=audioInput.files[0];for(const f of photos){const u=URL.createObjectURL(f);const el=document.createElement('img');el.src=u;el.onload=()=>URL.revokeObjectURL(u);preview.appendChild(el)}if(audio){const chip=document.createElement('div');chip.className='upload-file-chip';chip.textContent=`♫ ${audio.name}`;preview.appendChild(chip)}}
    async function previewYouTube(){ytPreview.innerHTML='';const v=parseYouTube(youtubeInput.value);if(!v)return;const enriched=await enrichYouTube(v);ytPreview.innerHTML=`<div class="yt-input-preview"><img src="${esc(enriched.thumbnail)}" alt=""><div><span class="eyebrow">YouTube</span><strong>${esc(enriched.title||'видео готово')}</strong><small>ролик будет встроен без загрузки файла</small></div></div>`;}
    photosInput.addEventListener('change',previewUploads);audioInput.addEventListener('change',previewUploads);youtubeInput.addEventListener('input',()=>{clearTimeout(youtubeInput._timer);youtubeInput._timer=setTimeout(previewYouTube,450);});
    async function saveEntry(){
      const title=$('entryTitle').value.trim(),text=$('entryText').value.trim(),date=$('entryDate').value.trim(),youtubeUrl=youtubeInput.value.trim();const photos=[...photosInput.files].slice(0,MAX_IMAGES),audio=audioInput.files[0];let youtube=youtubeUrl?parseYouTube(youtubeUrl):null;
      if(!title&&!text&&!photos.length&&!youtube&&!audio){alert('Добавьте хотя бы что-нибудь в новую главу.');return}
      if(audio&&audio.size>MAX_AUDIO_BYTES){alert(`Песня слишком большая. Лимит этой версии: ${CFG.maxAudioMB||60} МБ.`);return}
      const btn=$('entrySaveBtn');btn.disabled=true;btn.textContent='сохраняю…';const id=uid();const paths={photos:[],audio:''};
      try{if(!supabaseClient)throw new Error('Музей не подключён к облаку. Проверь config.js.');
        if(youtube){setProgress(progress,'проверяю YouTube…');youtube=await enrichYouTube(youtube);youtube.added_at=new Date().toISOString();}
        ...
        setProgress(progress,'создаю запись…');const payload={id,room_id:ROOM,date_label:date,title,body:text,photo_paths:paths.photos,video_paths:[],youtube_videos:youtube?[youtube]:[],audio_path:paths.audio,created_by:currentUser?.id||null};const {data,error}=await supabaseClient.from(TABLE).insert(payload).select().single();if(error)throw error;
      ...
      }catch(e){console.error(e);await deleteStorage([...paths.photos,...(paths.audio?[paths.audio]:[])]);alert('Не удалось сохранить главу. Проверь подключение Supabase и попробуй ещё раз.\n\n'+(e.message||''));}finally{btn.disabled=false;btn.textContent='сохранить главу';}
    }
    async function openView(e){const media=e.media||await mediaUrls(e);e.media=media;const yts=(e.youtubeVideos||[]);const c=$('museumViewContent');c.innerHTML=`<div class="museum-view-date">${esc(e.date||new Date(e.createdAt).toLocaleDateString('ru-RU'))}</div><div class="museum-view-title">${esc(e.title||'без названия')}</div>${media.photos.length?`<div class="museum-view-photos">${media.photos.map(u=>`<img src="${u}" alt="" loading="lazy">`).join('')}</div>`:''}${yts.length?`<div class="museum-view-youtube">${yts.map(v=>youtubeEmbed(v.video_id)).join('')}</div>`:''}${media.videos.length?`<div class="museum-view-videos">${media.videos.map(u=>`<video src="${u}" controls playsinline preload="metadata"></video>`).join('')}</div>`:''}<div class="museum-view-text">${esc(e.text||'')}</div>${yts.length&&yts[0].title?`<div class="youtube-caption"><span>YouTube</span><strong>${esc(yts[0].title)}</strong>${yts[0].author_name?`<small>${esc(yts[0].author_name)}</small>`:''}</div>`:''}${media.audio?`<div class="event-song"><strong>♫ песня этого события</strong><audio src="${media.audio}" controls preload="metadata"></audio></div>`:''}<div class="museum-view-actions"><button class="museum-delete-btn" id="museumDeleteBtn">удалить главу</button></div>`;c.querySelector('#museumDeleteBtn')?.addEventListener('click',async()=>{if(!confirm('Удалить главу, фотографии, старые видео и песню?'))return;try{await deleteStorage([...e.photos,...e.videos,...(e.audioPath?[e.audioPath]:[])]);if(supabaseClient){const {error}=await supabaseClient.from(TABLE).delete().eq('id',e.id).eq('room_id',ROOM);if(error)throw error}memories=memories.filter(x=>x.id!==e.id);backupWrite(memories);render();viewModal.classList.remove('show')}catch(err){console.error(err);alert('Не удалось удалить главу.')}});viewModal.classList.add('show');}
    function renderCinema(){const cg=$('cinemaGrid'),ce=$('cinemaEmpty');if(!cg)return;const items=[];for(const e of memories)for(const v of (e.youtubeVideos||[]))items.push({...v,memoryTitle:e.title,memoryDate:e.date});cg.innerHTML=items.map(v=>youtubeCard(v)).join('');ce.style.display=items.length?'none':'block';bindYouTubePosters(cg);}
    $('museumAddBtn').addEventListener('click',openAdd);$('museumRefreshBtn').addEventListener('click',refresh);$('museumRandomBtn').addEventListener('click',()=>memories.length&&openView(memories[Math.floor(Math.random()*memories.length)]));$('entrySaveBtn').addEventListener('click',saveEntry);$('museumExportBtn').addEventListener('click',()=>{const data=memories.map(({media,...e})=>e);const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:'application/json'}));a.download=`our-story-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)});$('museumImportBtn').addEventListener('click',()=>$('museumImportFile').click());$('museumImportFile').addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;try{const incoming=JSON.parse(await f.text());if(!Array.isArray(incoming))throw new Error('bad backup');if(!supabaseClient)throw new Error('Для импорта в общий музей нужен Supabase');let added=0;for(const x of incoming){if(!x?.id||memories.some(m=>m.id===x.id))continue;const payload={id:x.id,room_id:ROOM,date_label:x.date||'',title:x.title||'',body:x.text||'',photo_paths:Array.isArray(x.photos)?x.photos:[],video_paths:Array.isArray(x.videos)?x.videos:[],youtube_videos:Array.isArray(x.youtubeVideos)?x.youtubeVideos:[],audio_path:x.audioPath||'',created_by:currentUser?.id||null};const {error}=await supabaseClient.from(TABLE).insert(payload);if(!error)added++}await refresh();alert(`Импорт завершён. Добавлено: ${added}.`)}catch(err){console.error(err);alert('Не удалось восстановить резервную копию.\n'+(err.message||''))}finally{e.target.value=''}});
    document.querySelectorAll('[data-close]').forEach(b=>b.addEventListener('click',()=>document.getElementById(b.dataset.close)?.classList.remove('show')));[modal,viewModal].forEach(m=>m.addEventListener('click',e=>{if(e.target===m)m.classList.remove('show')}));await refresh();
  }
